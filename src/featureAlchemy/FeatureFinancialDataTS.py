...
class FeatureFinancialDataTS(IFeature):
    ...
    def __operateOnPriceData(self):
        ...
        # TODO: IN THIS FORM NOT USFUL ON FUTURE PREDICTION. 
    ...
    def __operateOnPriceData_lag(self, lagList: List[int] = []):
        #add lagged metrics
        columns_toLag = ["log_trailing_pe_ratio", "log_forward_pe_ratio", "log_pb_ratio"]
        for lag in lagList:
            self.shareprice = self.shareprice.with_columns([
            ...
            self.columns_toFeature_metric += (
                ...
                [f"{col}_lagquot_m{lag}" for col in columns_toLag]
            )
