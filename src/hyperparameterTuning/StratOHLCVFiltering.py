...
class StratOHLCVFiltering(BaseStrategy):
    ...
    def sample_params(self, trial: optuna.Trial) -> dict:
        ...
        opt_params["adjhigh_qup"]                    = trial.suggest_float("adjhigh_qup", 0.01, 0.94)
        min_down = min(opt_params["adjhigh_qup"] + 0.05, 0.96)
        max_down = min(min_down + 0.15, 0.999)
        opt_params["adjhigh_qdown"]                  = trial.suggest_float("adjhigh_qdown", min_down, max_down)
        opt_params["adjhigh_idxdays"]                = trial.suggest_int("adjhigh_idxdays", 1, 4)
        opt_params["adjhigh_decay"]                  = trial.suggest_float("adjhigh_decay", 0.5, 2.0)
    ...
    def run(
        ...
        Xte_time_adjhigh_window = Xte_time[:, -idxdays_high:, lstm_idx_adjhigh]
        #Xte_time_adjlow_window = Xte_time[:, -idxdays_low:, lstm_idx_adjlow]
        ...
        Xte_time_adjhigh_vec = np.sum(Xte_time_adjhigh_window * weights_high, axis=1)
        ...
        #Xte_time_adjlow_vec = np.sum(Xte_time_adjlow_window * weights_low, axis=1)

        # Quantiles
        qup_high_val   = np.quantile(Xtr_time_adjhigh_vec, qup_high)
        qdown_high_val = np.quantile(Xtr_time_adjhigh_vec, qdown_high)
        masktr_qup_high   = Xtr_time_adjhigh_vec >= qup_high_val
        masktr_qdown_high = Xtr_time_adjhigh_vec <= qdown_high_val
        maskte_qup_high   = Xte_time_adjhigh_vec >= qup_high_val
        maskte_qdown_high = Xte_time_adjhigh_vec <= qdown_high_val

        #qup_low_val   = np.quantile(Xtr_time_adjlow_vec, qup_low)
        #qdown_low_val = np.quantile(Xtr_time_adjlow_vec, qdown_low)
        #masktr_qup_low   = Xtr_time_adjlow_vec > qup_low_val
        #masktr_qdown_low = Xtr_time_adjlow_vec < qdown_low_val
        #maskte_qup_low   = Xte_time_adjlow_vec > qup_low_val
        #maskte_qdown_low = Xte_time_adjlow_vec < qdown_low_val

        # Final masks
        mask_tr_high = np.logical_and(masktr_qup_high, masktr_qdown_high)
        #mask_tr_low = np.logical_and(masktr_qup_low, masktr_qdown_low)
        
        mask_te_high = np.logical_and(maskte_qup_high, maskte_qdown_high)
        #mask_te_low = np.logical_and(maskte_qup_low, maskte_qdown_low)
        
        mask_train = np.logical_and(mask_train, mask_tr_high)
        
        mask_tar = np.logical_and(mask_test, mask_te_high)

        #n_selected = np.count_nonzero(mask_tar)
        #if n_selected < min_n_tar:
        #    remaining_idx = np.where(np.logical_and(mask_test, ~mask_tar))[0]
        ...
        logger.info(f"  Run -> train kept: {mask_train.mean()} | test kept: {mask_test.mean()}")
        logger.info(f"  Run -> mask_train sum: {mask_train.sum()} | mask_test sum: {mask_test.sum()}")
