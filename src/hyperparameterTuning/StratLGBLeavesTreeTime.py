...
class StratLGBLeavesTreeTime(BaseStrategy):
    ...
    def sample_params(self, trial: optuna.Trial) -> dict:
        ...
        opt_params["LGB_lambda_l1"]                 = trial.suggest_float("LGB_lambda_l1", 0.00001, 0.00005, log=True)
        opt_params["LGB_lambda_l2"]                 = trial.suggest_float("LGB_lambda_l2", 0.000001, 0.001, log=True)
        ...
        if opt_params["apply_feature_fraction"]:
            opt_params["LGB_feature_fraction"]      = trial.suggest_float("LGB_feature_fraction", 0.95, 0.99, log=False)
        opt_params["LGB_num_leaves"]                = trial.suggest_int("LGB_num_leaves", 300, 2300, step=5)
        opt_params["LGB_max_depth"]                 = trial.suggest_int("LGB_max_depth", 9, 23, step=1)
        opt_params["LGB_learning_rate"]             = 0.0001 #trial.suggest_float("LGB_learning_rate", 1e-5, 2e-0, log=True)
        opt_params["LGB_min_data_in_leaf"]          = trial.suggest_int("LGB_min_data_in_leaf", 350, 700, step=1)
        opt_params["LGB_min_gain_to_split"]         = trial.suggest_float("LGB_min_gain_to_split", 1e-6, 5e-2, log=True)
        ...
        opt_params["LGB_max_bin"]                   = trial.suggest_int("LGB_max_bin", 150, 350, step=10)
        ...
        opt_params["top_n_max"]         = trial.suggest_int("top_n_max", 50, 650, step=25)  
        ...
        opt_params["ytr_opt_alpha"] = trial.suggest_float("ytr_opt_alpha", 0.01, 0.2, log=True)

        opt_params["t_win"] = trial.suggest_int("t_win", 18, 80, step=1)
    ...
    def run(
        ...
        if meta_test.is_empty() or meta_test['date'].is_empty():
            return np.zeros(Xte_tree.shape[0], dtype=bool), 0.88*np.ones(Xte_tree.shape[0], dtype=float), 2.0*np.ones(Xte_tree.shape[0], dtype=float)
        ...
        logger.info(f"  n last day test samples: {last_day_mask.to_numpy().sum()}")
        logger.info(f"  n res_mask test samples: {res_mask.sum()}")
