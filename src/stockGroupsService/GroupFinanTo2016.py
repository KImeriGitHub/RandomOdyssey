...
class GroupFinanTo2016(IGroup):
  ...
  def checkAsset(self, asset: AssetData) -> bool:
    ...
    buffer_quar = 10
    buffer_ann = 10
    if asset.financials_quarterly[GroupFinanTo2016.quarterly_columns].tail(quarterly_entries).isnull().sum().sum() > buffer_quar:
      return False
    if asset.financials_annually[GroupFinanTo2016.annual_columns].tail(annual_entries).isnull().sum().sum() > buffer_ann:
      return False
    if all([asset.financials_quarterly[col].tail(quarterly_entries).isnull().sum() > 0 for col in GroupFinanTo2016.quarterly_columns]):
      return False
    if all([asset.financials_annually[col].tail(annual_entries).isnull().sum() > 0 for col in GroupFinanTo2016.annual_columns]):
      return False
